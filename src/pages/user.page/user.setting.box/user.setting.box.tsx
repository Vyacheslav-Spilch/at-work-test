...
import { IUser } from "../../../store/api/api"

export const UserSettingBox = ( { user }: { user: IUser }) => {

    const categoriesList = [ "Данные профиля", "Рабочее пространство", "Приватность", "Безопасность"]
        ...
        <section className={s.container_setting}>
            ...
            <div className={s.box_categories}> 
                {categoriesList.map((category) => (
                    <div key={category} className={s.category}>{category}</div>
                ))}
            </div>
        </section>
    )

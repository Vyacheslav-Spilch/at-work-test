...
import { userFieldList, userFieldsSchema, UserFormType } from "./validation/validation";
import { useAppDispatch } from "../../../store/hooks";
import { BoxField } from "../../../components/box.field/box.field";
...
export const UserFormFields = ({ user }: { user: IUser }) => {
    ...
    const { register, handleSubmit, setValue, formState: { errors }} = useForm<UserFormType>({
    ...
    useEffect(() => {
      ...
      const onHandleSubmitUserData = (data: UserFormType) => {
        if(CheckForSpaces(data) && user) {
            dispatch(updateUserInfo({ user: { ...data, id: user.id, archive: user.archive}}))
